...
const SelectTrigger = React.forwardRef(
  ({ className, children, iconColor, ...props }, ref) => (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex h-10 w-full items-center justify-between rounded-md border border-slate-200 bg-white px-3 py-2 text-sm  placeholder:text-slate-500 focus:outline-none   disabled:cursor-not-allowed disabled:opacity-50 ",
        className
      )}
      {...props}
    >
      ...
      <SelectPrimitive.Icon asChild>
        <DropdownIcon
          className={cn(
            "h-3 w-3",
            iconColor === "black" ? "fill-black opacity-70" : "fill-white"
          )}
        />
...
const SelectItem = React.forwardRef(
  ({ className, children, check, ...props }, ref) => (
